...
/** Where §7's re-application goes. A second attempt is a second CASE. */
const SECOND_CASE = caseId("case_demo_002");
...
async function main(): Promise<void> {
  ...
  if (instructed) {
    ...
    // ── The instruction opens a NEW case (ADR-0006 §3, as amended in P38) ──
    //
    // Not a new ordinal on this one. A second attempt is genuinely a different
    // application — different intake, different deadline, possibly changed
    // entry requirements, and a separate authorisation — and one case holding
    // two of each could not say precisely what the student agreed to.
    //
    // It is also the only shape that WORKS: CONFIRMED is terminal, terminal
    // states have no outgoing transitions, and a case whose ordinal was bumped
    // in place would sit at attempt 2 unable to reach `PREPARING`.
    const prior = await currentCase();
    const instruction = (await store.read(CASE))
      .filter((event) => isEventOfType(event, "ReapplicationInstructed"))
      .at(-1);
    if (instruction === undefined) throw new Error("the instruction was just appended");

    const second = openReapplication({
      priorCase: prior,
      instructed: instruction,
      requestEvidence: {
        requestedAt: CLOCK,
        channel: "aas_conversation",
        conversationRef: externalRef("askimate:conversation:9932"),
        studentStatement: "I understand, but I'd like to apply again for the same intake.",
      },
    });
    const opening = stamp({
      ...
      actor: ACTOR,
      now: CLOCK,
      nextEventId: () => "evt_r0001",
    });
    await store.append(SECOND_CASE, 0, opening);
    const attempt2 = fold(await store.read(SECOND_CASE));

    ok(`A second case, ${SECOND_CASE}, at attempt ${String(attempt2.submissionIdentity.attemptOrdinal)}`);
    note(`It names the application it follows: ${String(attempt2.priorCaseId)}`);
    await store.claimSubmissionKey(submissionKey(attempt2.submissionIdentity), SECOND_CASE);
    note("Its own submission key, claimed — a different attempt is a different key.");

    ok(`The first case is untouched: ${prior.state}, attempt ${String(prior.submissionIdentity.attemptOrdinal)}`);
    note("Its authorisation and its submission still describe what actually happened.");

    // ── And the second case can ACT, which the old shape could not ──────
    //
    // The load-bearing claim of the whole change. Bumping the ordinal on the
    // prior case produced a CONFIRMED case at attempt 2, and `checkTransition`
    // refuses from a terminal state before it looks at the target — so the
    // "new attempt" had no first move. This one starts where any case starts.
    const moves = decide(attempt2, {
      kind: "transition",
      to: "READY_TO_PREPARE",
      reason: "Second attempt, starting from the top.",
    });
    if (moves.accepted) {
      ok("And it can act — a terminal case bumped in place never could");
    } else {
      brokenPromises.push(
        `the second case could not take its first step — ${JSON.stringify(moves.refusal)}`,
      );
    }
  }
  ...
  console.log(
    `  Re-applied      ${
      final.reapplication === undefined
        ? "no"
        : `yes — ${final.reapplication.newCaseId}, attempt ${String(final.reapplication.newAttemptOrdinal)}`
    }`,
  );
  console.log();
