/**
 * The journey through the five REAL processes, on an account the student
 * already holds (P122, ADR-0110).
 *
 * ═══════════════════════════════════════════════════════════════════════════
...
const PORT_BASE = 4970;
const PREFIX = "aas_localstack_existing";
...
beforeAll(async () => {
  ...
  // ── The account exists before the run does, and it is the student's ─────
  //
  // Made on the portal by "the student" — a plain form post, as a person's
  // browser would make it — with the password only they know. The run is told
  // it exists and signs in; it never registers.
  const registered = await fetch(`${portal.baseUrl}/register`, {
    method: "POST",
    headers: { "content-type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({ email: EMAIL, password: PASSWORD, password_confirm: PASSWORD }),
    redirect: "manual",
  });
  if (registered.status !== 302) throw new Error(`the portal did not register the student's own account: ${String(registered.status)}`);
  if (portal.accounts().length !== 1) throw new Error("the portal should hold exactly the student's account");
...
describeIfBoth("the journey through the five processes the local-stack script starts", () => {
  ...
  it("the student opens a conversation, is offered the target, asks for it, and is asked to authorise", async () => {
    ...
    // ADR-0110: before the yes, the student says the account is theirs.
    const declared = await asStudent(`/v1/conversations/${conversation}/runs/${runId}/decision`, { method: "POST", body: { kind: "existing_account" } });
    expect(declared.status, await declared.clone().text()).toBe(204);
    expect(portal.accounts(), "the one account, theirs, from before the run").toEqual([EMAIL]);
  }, 120_000);
  ...
  it("the student reads the preview and says yes; the WORKER PROCESS moves the run to the password without another request", async () => {
    ...
    expect(portal.accounts(), "still exactly theirs").toEqual([EMAIL]);
    // The ask is the sign-in, said as a start: the case carries the declaration.
    const declared = await conversationPool.query(
      "SELECT 1 FROM case_events WHERE event->>'type' = 'PortalAccountDeclared'",
    );
    expect(declared.rowCount).toBe(1);
  }, 180_000);

  it("the password goes through the REAL frame; the runner PROCESS signs in to the student's own account, creates none, fills the portal; the run asks for the account back", async () => {
    ...
    try {
      ...
      // A sign-in asks for the password once: the confirmation box a creation
    ...
    expect(portal.accounts(), "still the one account: nothing was registered").toEqual([EMAIL]);
    expect(portal.credentialsWork(EMAIL, PASSWORD), "with the password only they know").toBe(true);
    const created = await conversationPool.query(
      "SELECT 1 FROM workflow_action_intents WHERE run_id = $1 AND action = 'create_portal_account'",
      [runId],
    );
    expect(created.rowCount, "no creation intent was ever opened").toBe(0);
  ...
  it("the student confirms ONCE that they can sign in — no reset of a password that was always theirs — and the run finishes ready to submit, nothing submitted", async () => {
    // Vahid's waiver, 2026-09-14: the reset proof is for an account we made,
    // where the address was never tested. This one they made and sign in with.
