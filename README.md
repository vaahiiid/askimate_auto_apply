...
| | |
|---|---|
| **Phase** | P195 — **blocker 61 closed: the country table is a reviewed artefact** ([ADR-0141](./docs/decisions/0141-the-country-table-is-a-reviewed-artefact.md)). ISO 3166-1 alpha-2, derived from ICU minus 31 named non-assignments — 280 − 31 = 249, and a test re-runs the derivation — canonicalised and hashed so an edit cannot pass unnoticed. **Membership is checked, not shape**, so `ZZ` is refused; and a name resolves, so a student types *Iran* rather than looking up their own country's code. Before it, P194 closed blocker 60 |
