...
| | |
|---|---|
| **Phase** | P194 — **blocker 60 closed: a field with several parts survives the request that asked for it.** The conversation log carries `value_part_read` ([ADR-0140](./docs/decisions/0140-a-field-with-several-parts-is-asked-part-by-part.md) §7, migration 0024) — its own event kind, so `open_value_proposals` cannot report half an address as an outstanding confirmation. P192's stop is gone, and its removal is held by the same guard that named it. Before it, P193's four decisions, including **a minor is not served until the guardian route is real** |
...
| **Tests** | **2,834 passing · 145 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
