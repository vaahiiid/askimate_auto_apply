...
| | |
|---|---|
| **Phase** | P200 — **the country-mapping derivation, and a review page that asks for a reading rather than a rubber stamp**. The two Sheffield selects that submit ISO codes derive at 235 of 249 with nothing guessed; the four that submit names miss 15–18% *silently* — `IR` among them — so they go to a person. [`docs/run-a/country-mapping-review.md`](./docs/run-a/country-mapping-review.md) carries the disagreements in full, with a spot-check anyone can repeat by re-running `pnpm run country-mappings` |
...
| **Tests** | **2,873 passing · 147 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
