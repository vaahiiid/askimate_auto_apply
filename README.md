...
| | |
|---|---|
| **Phase** | P198 — **a red suite that said nothing**. CI 327 failed on a records-only commit with `expected 'Loading…' to contain 'received'` — the placeholder the secure frame ships with, which covers both *the password never went* and *the password went and the page never said so*. It was the second, and proving that took reading the five other tests in the same file. The wait is made in four places and now shares one helper, which on failure reports the frame's state, its own error line, how many frames are still mounted, and the run as the service reports it. The twenty-second ceilings are deliberately unchanged — that would be fixing the assertion rather than the contention (blocker 67) |
...
| **Tests** | **2,853 passing · 146 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
| **Decisions** | 141 ADRs · all 141 Accepted |
...
implementation, no deployable holding a vault. `national_id` was removed from the supported document
...
scripts/local-stack.sh start   # the five deployables on this machine, against your Postgres and Redis — docs/runbook-local-stack.md
```
