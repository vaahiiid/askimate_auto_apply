...
| | |
|---|---|
| **Phase** | P196 — four records before the composites, and **a correction**: P195's reason for holding blocker 64 was wrong — the content hash covers the entry, not the profile, so no signature is spent. Measurement also found the three "free-text" country fields **already hold ISO codes seeded by hand**, and the interview would store text the mapping cannot match. Also recorded: a country has **two** mappings, ours and each portal's (Sheffield submits four distinct values for Iran, none of them `IR`); and a country a portal does not offer is **not a parse failure** — named as blocker 66 |
...
| **Tests** | **2,845 passing · 146 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
