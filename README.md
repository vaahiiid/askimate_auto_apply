...
| | |
|---|---|
| **Phase** | P197 — **the three remaining composites**: the language test, the seven UK-status claims, previous UK study. A component score is **never inferred from the overall**; a result is kept as the certificate writes it rather than parsed to a number; *"about 3 years"* on a visa is refused. The interview can now ask for every ordinary registry field except the five list-valued keys and `education.highest_qualification` — the class held pending Part 2, and a test asserts that exact list |
