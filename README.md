...
| | |
|---|---|
| **Phase** | P202 — **the review page had a trap in it, and he found it**: one Sheffield option, *Congo*, proposed to both `CD` and `CG`. A collision rule withdraws any submitted value proposed for two countries, and a second pass searches the portal's list by the portal's **own** name for a code — Sheffield calls `CZ` *Czech Republic* in its code list, so the name was there all along. What a reviewer must read fell from 37–44 a field to **4–6** |
...
| **Tests** | **2,886 passing · 147 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
