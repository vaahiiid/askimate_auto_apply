...
| | |
|---|---|
| **Phase** | P207 — **the entry is signed** (`f67ec69`, `sha256:f13dff6d…`, Vahid's own hash computation): nine country maps, 2,088 values, and the four spellings of Iran now asserted from the signed entry rather than checked by eye. **Four of the six remaining partial maps hold exactly one row, and Run A's value is that row.** Costed per map in a decision sheet — and `degree` turns out not to be partial but **wrong**: it maps a level onto an award title, so `Bachelor's degree → BSc` renders silently wrong for every BA |
