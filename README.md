...
| | |
|---|---|
| **Phase** | P208 — **blocker 71 raised at the top of the list: the `degree` map tells a university something untrue.** It reads a *level* and writes an *award title*, so every BA student is told they hold a BSc — and nothing refuses: plan blockers 0, validator violations 0, preview builds, read-back passes, because the value lands. **Run A passed through it**, its profile holding a BSc. The stop is measured and needs no code: emptying the map gives **three independent refusals** three steps before anything could be sent |
