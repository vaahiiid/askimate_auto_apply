...
| | |
|---|---|
| **Phase** | P203 — **two pairings that would have put the wrong country on an application**, caught by a person reading the page: `MP` *Northern Mariana Islands* → *Northern Ireland*, `TF` *French Southern Territories* → *French West Indies*, each from one shared word. The proposer's bar is now two shared words or a word-for-word opening, it explains itself on the page, and his twelve verdicts are a file the derivation reads — **eight rows left to read**, from 37–44 a field two phases ago |
...
| **Tests** | **2,897 passing · 147 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
