...
| | |
|---|---|
| **Phase** | P199 — **blocker 64 taken, and the reviewed entry turned out to be the part that was right**. The three country fields now read through the reviewed ISO table, so *Iran* stores `IR`; the signed Sheffield entry already carried the per-field portal spellings for all nine of its country mappings, so **no signature was spent**. The fix exposed the larger thing: a student's ordinary answer — *"Iranian"* — is refused three times and escalated, ICU ships no demonyms, and the reviewed mapping set carries 8 countries of 249 |
...
| **Tests** | **2,859 passing · 146 files · zero skipped**, in two lanes — browsers serial, everything else parallel · typecheck, lint, boundary, reachability and contract checks green |
