...
the sixth, a research build, was removed in P53 (ADR-0086). **2,247 tests, 117 files, zero skipped**, against real PostgreSQL and Redis, in two lanes —
...
| **16** | **One S3 bucket and a prefix-scoped credential, to verify the checksum binding** | You — AWS spend is your act | **The durable document store (ADR-0092).** The port is not reshaped until `pnpm run verify-s3-checksum` says VERIFIED against a real bucket. Exactly what must exist, what it costs and what it can reach: `docs/provisioning-request-s3-verification.md`. Nothing has been created |
...
**2,247 tests · 117 files · zero skipped · zero pending**, run against real PostgreSQL 16 and real
...
**2,247 tests**, by the workspace they live in. Generated — run
...
| `scripts` | 274 | `packages/profile` | 46 |
