...
| **22** | **A save pressed is reported as a save made** — after the advance control is pressed the runner reads nothing back (`fill-application.ts`, the click resolves on the press), so a save the portal refused on the same page is reported *succeeded* and its transmissions joined to it. Surfaced by Vahid's 2026-09-12 observation that Sheffield's save lands on *Your Details*, and by the open question whether its education page saves with a radio unanswered | You — the shape; then me | The truth of every page report on a real fill, and the ADR-0105 shape if the real form requires each radio answered. Proposal: the blueprint names how a saved page looks (a landing URL, or an error locator that must be absent); the runner reads it after the press and reports *refused* or *uncertain* when it does not hold |
