...
the sixth, a research build, was removed in P53 (ADR-0086). **2,249 tests, 117 files, zero skipped**, against real PostgreSQL and Redis, in two lanes —
...
| **16** | **Billing alerts, one S3 bucket, the vault's CMK and a prefix-scoped credential, to verify the checksum binding AND SSE-KMS through a pre-signed PUT** | You — AWS spend is your act. **Approved 2026-09-09: "I am creating it."** | **The durable document store (ADR-0092).** The port is not reshaped until `pnpm run verify-s3-checksum` says VERIFIED on **both halves** against a real bucket — the KMS half made required by you: *"if a pre-signed PUT cannot carry SSE-KMS under a CMK the uploader has no grant to, then D has a hole in it."* Exactly what must exist, what it costs and what it can reach: `docs/provisioning-request-s3-verification.md`. Nothing has been created by the agent; nothing runs until you say the variables are set |
...
**2,249 tests · 117 files · zero skipped · zero pending**, run against real PostgreSQL 16 and real
...
**2,249 tests**, by the workspace they live in. Generated — run
...
| `scripts` | 276 | `packages/profile` | 46 |
