...
processes. **2,315 tests, 117 files, zero skipped**, against real PostgreSQL and Redis, in two lanes —
the seventeen files that launch a browser run serially, everything else in parallel.
Eighty-four architecture decision records, all eighty-four accepted (ADR-0006 §3 amended in P38). **£0 / $0 of the ~$1,000 AWS credit is spent —
...
**2,315 tests · 117 files · zero skipped · zero pending**, run against real PostgreSQL 16 and real
...
**2,315 tests**, by the workspace they live in. Generated — run
...
| `scripts` | 266 | `packages/profile` | 46 |
