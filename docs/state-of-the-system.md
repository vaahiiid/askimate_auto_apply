...
| **21** | **A document per item of a repeating page, and a condition inside one** — Sheffield's education page carries six document slots per qualification and a field shown by another; ADR-0103 gap 3 as built refuses both on a repeating page, rightly (a document is mapped to a held type, not to an item; a condition inside a repeat has no item to be answered by), so the page the gap was raised for cannot be marked. Sheet: [`decision-sheet-blocker-21-a-document-per-item-of-a-repeating-page.md`](./decision-sheet-blocker-21-a-document-per-item-of-a-repeating-page.md) — three options (the profile holds documents per item; the documents are the student's own act on that page; the page is not marked), a recommendation (B now, A when the registry holds education), and the per-item condition as a separate smaller piece | You — a product question about what a student holds per qualification, before a schema one | The fill of the first real form's education page; the Sheffield draft's `page7` stays unmarked; nothing is built against it until decided |
