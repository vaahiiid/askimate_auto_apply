...
the sixth, a research build, was removed in P53 (ADR-0086). **2,514 tests, 130 files, zero skipped**, against real PostgreSQL and Redis, in two lanes —
...
| **22** | **A page is saved when the portal shows it, not when a control is pressed** — after the advance control is pressed the runner reads nothing back and reports *succeeded*. Met on the real portal by Vahid, 2026-09-12: a second qualification saved with two radios unanswered drew *no error* and *summary.do shows Previous Education 1 and nothing else* — silently dropped. His shape: *"It is reported saved when the portal shows the thing exists. Where a blueprint can name how to see that, name it. Where it cannot, the honest report is uncertain, not succeeded."* Sheet: [`decision-sheet-blocker-22-a-page-is-saved-when-the-portal-shows-it.md`](./decision-sheet-blocker-22-a-page-is-saved-when-the-portal-shows-it.md) | You — the shape, in your words; then me, three phases | The truth of every page report on a real fill, and every transmission record on a page whose save was dropped. The cost, answered: `uncertain` is built end to end and costs a specialist's look per page; a reopen-and-read costs the reviewer nothing and covers most non-repeating pages; a listing count and a slot marker are named vocabulary; a portal with nothing to read is a look per page |
...
**2,514 tests · 130 files · zero skipped · zero pending**, run against real PostgreSQL 16 and real
...
**2,514 tests**, by the workspace they live in. Generated — run
...
|---|---|---|---|
| `apps/conversation-service` | 413 | `packages/mapping` | 61 |
| `packages/domain` | 376 | `packages/conversation` | 52 |
| `scripts` | 306 | `packages/preparation` | 52 |
| `apps/browser-runner` | 287 | `packages/profile` | 49 |
| `packages/case-store` | 145 | `packages/disclosure` | 47 |
| `packages/orchestrator` | 122 | `packages/catalogue` | 44 |
