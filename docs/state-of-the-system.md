...
| **20** | **A field the portal asks for that our rules forbid us to hold** — Sheffield's equal-opportunities page: eleven disability checkboxes, a support-needs box, an ethnic-origin select (Article 9). Sheet: [`decision-sheet-article-9-fields-a-portal-asks-for.md`](./decision-sheet-article-9-fields-a-portal-asks-for.md) | You — *"That is a decision, not a defect, and it is mine"* (2026-09-11) | The mapping set for the first real blueprint: which fields are left to the student, and how the preview says so. Recommendation A; depends on whether the page saves blank, which only the first real fill shows |
