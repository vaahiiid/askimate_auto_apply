...
the sixth, a research build, was removed in P53 (ADR-0086). **2,624 tests, 140 files, zero skipped**, against real PostgreSQL and Redis, in two lanes —
the eighteen files that launch a browser run serially, everything else in parallel.
One hundred and eleven architecture decision records, all accepted (ADR-0006 §3 amended in P38). **AWS spend is no longer $0:** one bucket, one
...
| **26** | **A failed account creation is re-claimed without limit, and cannot succeed once the secret is spent** — observed on the P121 journey: the Runner reported `create_portal_account` failed, the Worker's next tick re-offered it, the Runner re-claimed about twice a second, and the Secure Service refused each use as `already_spent`; nothing caps the attempts, backs off, or asks the student for the password again. Raised in P121 (`docs/runbook-local-stack.md`, "Found by driving the journey through it") | You — in your words; then me | Any live run that fails at the account, against a real portal's registration page. Options named, none built: (A) cap attempts per intent and stop for a person; (B) on `already_spent` or `secret_unavailable`, open the secure box again as the resume path does (ADR-0101 §3); (C) both |
| **25** | **The institution box cannot be mapped by rule from a free-text profile field** — ADR-0109 makes a typeahead mapping name a recorded entry's value through an option rule; a student's institution is whatever they typed, and Sheffield's list is Sheffield's. Raised in P119 (`docs/distance-to-a-reviewed-sheffield-run.md`, item 5) | You — in your words; then me | The education page's mapping, and every portal with an institution lookup. Options named, none built: (A) the box as the student's own act; (B) an interview step offering the portal's entries; (C) a reviewed option map per institution |
...
**2,624 tests · 140 files · zero skipped · zero pending**, run against real PostgreSQL 16 and real
...
**2,624 tests**, by the workspace they live in. Generated — run
...
|---|---|---|---|
| `apps/conversation-service` | 414 | `packages/profile` | 57 |
| `packages/domain` | 378 | `packages/conversation` | 52 |
| `scripts` | 337 | `packages/preparation` | 52 |
| `apps/browser-runner` | 322 | `packages/disclosure` | 47 |
